...
#%% Configuration
print("="*70)
print("OPTUNA HYPERPARAMETER OPTIMIZATION - TRADITIONAL ML")
print("="*70)
...
print(f"OK Loaded test data: {X_test_tfidf.shape}")
...
def objective(trial):
    ...
    if best_model_name == 'LogisticRegression':
        ...
        # Ensure compatible solver-penalty combinations
        ...
        params = {
            ...
            'shrinkage': trial.suggest_categorical('shrinkage', [None, 'auto']) if trial.params.get('solver') in ['lsqr', 'eigen'] else None,
        }
...
study = optuna.create_study(
    direction='maximize',
    study_name='traditional_ml_recall_optimization',
...
if best_model_name == 'LogisticRegression':
    ...
    best_params['max_iter'] = 1000
    best_params['random_state'] = RANDOM_STATE
    optimized_model = SGDClassifier(**best_params)
...
optimized_model_file = os.path.join(DB_PATH, 'stage1_optimized_model.pkl')
...
results_file = os.path.join(GRAPHS_DIR, 'traditional_optimization_results.json')
with open(results_file, 'w') as f:
    json.dump(optimization_results, f, indent=2)
print(f"OK Saved optimization results to {results_file}")

# Save Optuna study
study_file = os.path.join(DB_PATH, 'traditional_optuna_study.pkl')
joblib.dump(study, study_file)
print(f"OK Saved Optuna study to {study_file}")

print("\n" + "="*70)
print("TRADITIONAL ML OPTIMIZATION COMPLETE")
print("="*70)
